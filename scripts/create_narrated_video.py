...
NODE_BOXES = {
    "Planner Agent":    (N["planner"][0]-NW//2,    N["planner"][1]-NH//2,
                         N["planner"][0]+NW//2,    N["planner"][1]+NH//2),
    "Research Agent":   (N["research"][0]-NW//2,   N["research"][1]-NH//2,
                         N["research"][0]+NW//2,   N["research"][1]+NH//2),
    "Analytics Agent":  (N["analytics"][0]-NW//2,  N["analytics"][1]-NH//2,
                         N["analytics"][0]+NW//2,  N["analytics"][1]+NH//2),
    "Critique Agent":   (N["critique"][0]-NW//2,   N["critique"][1]-NH//2,
                         N["critique"][0]+NW//2,   N["critique"][1]+NH//2),
    "HITL Checkpoint":  (N["hitl"][0]-NW//2,       N["hitl"][1]-NH//2,
                         N["hitl"][0]+NW//2,       N["hitl"][1]+NH//2),
    "Supervisor Agent": (N["supervisor"][0]-130,   N["supervisor"][1]-38,
                         N["supervisor"][0]+130,   N["supervisor"][1]+38),
}

NODE_ANNOTATIONS = {
    "Planner Agent":    ["Reads: query", "Writes: plan (5 sub-tasks)", "LLM: llama-3.3-70b"],
    "Research Agent":   ["Reads: plan", "Retrieves: top-4 chunks via RAG", "Source: 2,284 SEC EDGAR docs"],
    "Analytics Agent":  ["Reads: context + plan", "Writes: structured report", "Rule: grounded — no hallucination"],
    "Critique Agent":   ["LLM-as-Judge scoring", "Faithfulness · Coherence · Task", "Gate: overall >= 0.75"],
    "HITL Checkpoint":  ["interrupt() pauses graph", "State serialised to SQLite", "Resumes: approve / revise"],
    "Supervisor Agent": ["Meta-router node", "Evaluates state completeness", "Routes to next best agent"],
}
...
def draw_rounded_gradient(img: Image.Image, x1: int, y1: int, x2: int, y2: int,
                           top_c: tuple, bot_c: tuple, radius: int = 10,
                           border_c: tuple = None, border_w: int = 1) -> None:
    """Rounded rectangle with vertical gradient fill."""
    w, h = x2 - x1, y2 - y1
    tile = Image.new("RGB", (w, h), top_c)
    td   = ImageDraw.Draw(tile)
    for row in range(h):
        td.line([(0, row), (w, row)], fill=lerp_color(top_c, bot_c, row / max(h-1, 1)))
    ...
    if border_c:
        ...
        for i in range(border_w):
            d.rounded_rectangle([x1+i, y1+i, x2-i, y2-i], radius=radius, outline=border_c)


def draw_glow(img: Image.Image, x1: int, y1: int, x2: int, y2: int,
              color: tuple = (255, 213, 0), layers: int = 5) -> None:
    """
...
def arrowhead(draw: ImageDraw.Draw, tx: int, ty: int, angle_deg: float,
              size: int = 9, color: tuple = ARROW_CLR) -> None:
...
def draw_h_arrow(draw: ImageDraw.Draw, x1: int, y: int, x2: int,
                 color: tuple = ARROW_CLR, label: str = None) -> None:
    ...
    if label:
        ...
        tw = bb[2] - bb[0]
        draw.rectangle([lx-tw//2-3, ly-2, lx+tw//2+3, ly+bb[3]-bb[1]+2], fill=(18, 24, 36))
        draw.text((lx - tw//2, ly), label, fill=TXT_SEC, font=f)


def draw_v_arrow(draw: ImageDraw.Draw, x: int, y1: int, y2: int,
                 color: tuple = ARROW_CLR, label: str = None) -> None:
    ...
    if label:
        ...
        ly = (y1 + y2) // 2
        bb = draw.textbbox((0, 0), label, font=f)
        draw.rectangle([lx-2, ly-2, lx+bb[2]-bb[0]+4, ly+bb[3]-bb[1]+2], fill=(18, 24, 36))
        draw.text((lx, ly), label, fill=TXT_SEC, font=f)


def draw_l_arrow(draw: ImageDraw.Draw, points: list, color: tuple = ARROW_CLR,
                 label: str = None, label_pos: tuple = None) -> None:
    ...
    if label and label_pos:
        ...
        lx, ly = label_pos
        bb = draw.textbbox((0, 0), label, font=f)
        draw.rectangle([lx-3, ly-2, lx+bb[2]-bb[0]+4, ly+bb[3]-bb[1]+2], fill=(18, 24, 36))
        draw.text((lx, ly), label, fill=ACCENT, font=f)


def draw_node(img: Image.Image, cx: int, cy: int, nw: int, nh: int,
              emoji: str, title: str, subtitle: str, active: bool = False) -> None:
    x1, y1, x2, y2 = cx-nw//2, cy-nh//2, cx+nw//2, cy+nh//2
    if active:
        draw_glow(img, x1, y1, x2, y2)
    draw_rounded_gradient(img, x1, y1, x2, y2,
    ...
    d.text((cx-(eb[2]-eb[0])//2, y1+7),  emoji,    fill=ACCENT if active else TXT_PRI, font=fe)
    tb = d.textbbox((0, 0), title, font=ft)
    d.text((cx-(tb[2]-tb[0])//2, y1+34), title,    fill=TXT_PRI, font=ft)
    sb = d.textbbox((0, 0), subtitle, font=fs)
    d.text((cx-(sb[2]-sb[0])//2, y1+56), subtitle, fill=ACCENT if active else TXT_SEC, font=fs)
...
def draw_arch_base(img: Image.Image, active_label: str = None) -> None:
    ...
    # START badge
    sx, sy = 52, 265
    d.ellipse([sx-22, sy-22, sx+22, sy+22], fill=(18, 28, 44), outline=ARROW_CLR, width=2)
    fs2 = get_font(11, bold=True)
    sb2 = d.textbbox((0, 0), "START", font=fs2)
    d.text((sx-(sb2[2]-sb2[0])//2, sy-(sb2[3]-sb2[1])//2), "START", fill=ARROW_CLR, font=fs2)
    ...
    draw_l_arrow(d,
        ...
        color=SUCCESS, label="pass >= 0.75",
        label_pos=(N["critique"][0]+6, (N["critique"][1]+NH//2+N["hitl"][1])//2-8))
    ...
    draw_l_arrow(d,
        ...
        color=DANGER, label="revised -> retry",
        label_pos=(486, (N["hitl"][1]+N["analytics"][1])//2-8))

    draw_v_arrow(d, N["hitl"][0], N["hitl"][1]+NH//2, N["end"][1]-28,
    ...
    for i in range(steps):
        if i % 2 == 0:
            ...
            d.line([(int(sx0+(ex0-sx0)*t1), int(sy0+(ey0-sy0)*t1)),
                    (int(sx0+(ex0-sx0)*t2), int(sy0+(ey0-sy0)*t2))],
                   fill=(55, 75, 105), width=1)
    ...
    for key, _e, title, sub, lbl in nodes_def:
        ...
        draw_node(img, cx, cy, nw_s, nh_s, title[:1], title, sub,
                  active=(active_label == lbl))

    # END badge
    ecx, ecy = N["end"]
    draw_rounded_gradient(img, ecx-78, ecy-28, ecx+78, ecy+28,
                          (20, 52, 30), (12, 36, 20), radius=28,
                          border_c=SUCCESS, border_w=2)
    d2 = ImageDraw.Draw(img)
    fe2 = get_font(18, bold=True)
    eb2 = d2.textbbox((0, 0), "END", font=fe2)
    d2.text((ecx-(eb2[2]-eb2[0])//2, ecy-(eb2[3]-eb2[1])//2), "END",
            fill=SUCCESS, font=fe2)
    ...
    if active_label and active_label in NODE_ANNOTATIONS:
        ...
        draw_rounded_gradient(img, ann_x, ann_y, ann_x+bw, ann_y+bh,
                              (28, 40, 58), (18, 26, 40), radius=8,
                              border_c=ACCENT, border_w=1)
        d3 = ImageDraw.Draw(img)
        dot_x = bx2 if ann_x > bx2 else bx1
        d3.line([(dot_x, (by1+by2)//2),
                 (ann_x + (0 if ann_x > bx2 else bw), ann_y+bh//2)],
                fill=ACCENT, width=1)
        d3.ellipse([dot_x-3, (by1+by2)//2-3, dot_x+3, (by1+by2)//2+3], fill=ACCENT)
        for i, line in enumerate(lines):
            d3.text((ann_x+pad, ann_y+pad+i*lh), line,
                    fill=TXT_PRI if i == 0 else TXT_SEC, font=f_ann)
    ...
    d4.text((20, H-20),
            ...
            fill=(45, 65, 90), font=get_font(12))
...
def create_techstack_slide(output_path: str) -> None:
    ...
    def draw_card(img, x, y, cw, h1, h2, h3):
        ch = 92
        draw_rounded_gradient(img, x, y, x+cw, y+ch, NODE_TOP, NODE_BOT,
                              radius=10, border_c=NODE_BORDER, border_w=1)
    ...
    d2.text((20, H-20),
            ...
            fill=(45, 65, 90), font=get_font(13))
...
FRAMES = [
    {
        "img": "00a_architecture.png", "audio": "00a_narration.mp3",
        "boundaries": "00a_boundaries.json", "is_arch": True,
    ...
    {
        "img": "00b_techstack.png", "audio": "00b_narration.mp3",
        "boundaries": "00b_boundaries.json", "is_arch": False,
    ...
    {
        "img": "01_loaded.png", "audio": "01_narration.mp3",
        "boundaries": "01_boundaries.json", "is_arch": False,
    ...
    {
        "img": "02_query_typed.png", "audio": "02_narration.mp3",
        "boundaries": "02_boundaries.json", "is_arch": False,
    ...
    {
        "img": "03_running.png", "audio": "03_narration.mp3",
        "boundaries": "03_boundaries.json", "is_arch": False,
    ...
    {
        "img": "04_hitl_ready.png", "audio": "04_narration.mp3",
        "boundaries": "04_boundaries.json", "is_arch": False,
    ...
    {
        "img": "05_approved.png", "audio": "05_narration.mp3",
        "boundaries": "05_boundaries.json", "is_arch": False,
...
def make_canvas(img: Image.Image, caption: str, subtitle: str) -> Image.Image:
    ...
    d.text(((W-(cb[2]-cb[0]))//2, H+(CAPTION_HEIGHT-(cb[3]-cb[1]))//2),
           caption, fill=TXT_SEC, font=f_cap)
    draw_subtitle(d, subtitle, W, H)
    return canvas


def render_arch(caption: str, subtitle: str,
                highlights: list, t: float, duration: float) -> np.ndarray:
    img = Image.new("RGB", (W, H), BG)
...
def render_standard(img_path: str, caption: str, subtitle: str,
                    highlights: list, t: float, duration: float) -> np.ndarray:
...
def main() -> None:
    ...
    final.write_videofile(str(OUTPUT_VIDEO), fps=FPS,
